...
public class TfLiteMovenet {
    ...
    private final Size INPUT_SIZE = new Size(256, 256);
    private final int NUM_OF_KEYPOINTS = 17; // Movenet gives 17 keypoints [nose, left eye, right eye, left ear, right ear, left shoulder, right shoulder, left elbow, right elbow, left wrist, right wrist, left hip, right hip, left knee, right knee, left ankle, right ankle]
    private final int OUTPUT_PER_KPTS = 3; // for each keypoint, Movenet gives 3 values (x, y, score) , in the range [0;1]
    ...
    protected ByteBuffer convertBitmapToByteBuffer(Bitmap bitmap) {
        ...
        int pixel = 0;
        for (int i = 0; i < INPUT_SIZE.width; ++i) {
    ...
    public float[][][][] recognizeImage(Mat img) {
        ...
        if(img.rows()!=INPUT_SIZE.height || img.cols()!=INPUT_SIZE.width)
        {
            // resizing for Movenet model, with padding to keep ratio
            resizedWithScale = resizeWithPadding(img, (int)INPUT_SIZE.width, (int)INPUT_SIZE.height);
        }
        else
        {
        ...
        // Init output : the Movenet model outputs a map of Float32{1,1,17,3} =  17 keypoints x  3  values (x, y, score) , with x, y in the range [0;1]
        ouput = new float[1][1][NUM_OF_KEYPOINTS][OUTPUT_PER_KPTS];
        
        outputMap.put(0, ouput);
